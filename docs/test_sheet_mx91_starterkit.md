...
| [Sdcard Enviroment saving](#sdcard-environment-saving)                        |   KO    |
...
When we have environment boot the the environment reading sometimes fails
and we have the deafult environment which doesn't keep modifications.
...
Close J_USB1, B_M and nSD_BOOT jumpers and remove SD card to
enable serial download. Connect the board with the source
machine (i.e. the machine you are downloading the images from)
by using the board's OTG header (J25).
...
|   OK   |      UART 232 ttyLP1 (J21)    | see [note2](#note-2)
|   OK   |      UART 485 ttyLP4 (J22)    | see [note3](#note-3)
|   OK   |         Linux Console (J26)   |
|   KO   |              WIFI             | see [note4](#note-4)
|   KO   |           BLUETOOTH           | see [note5](#note-5)
|   N/A  |           MIPI-CSI (J18)      | see [note6](#note-6)
|   OK   |              RTC              | see [note7](#note-7)
|   KO   |            Reboot             | works only on eMMC
|   KO   |         Reboot button         | works only on eMMC
|   KO   |              LVDS             | see [note8](#note-8)
|   KO   |           Backlight           | see [note9](#note-9)
|   KO   |          Touchscreen         | see [note10](#note-10)
|   N/A  |             HDMI              |
|   KO   |             Audio             | see [note11](#note-11)
|   OK   |          CAN 0 (J23)          | see [note12](#note-12)
